...
| [pref.md](pref.md)                                 | User preferences on per-tenant Firestore (BC 2.0) or KV (BC 1.0 legacy): `useAppPrefs` / `useGlobalPrefs` / `useAppFeaturePrefs` / `useGlobalFeaturePrefs` (reactive objects, auto-debounced writes), `usePrefsStatus()` for backend status + lastError surfacing, local-FS dev fallback. Read when working on settings persistence. |
...
| [compute.md](compute.md)                           | Compute jobs (`jobs/<name>/`): cron, HTTP-triggered batch, sharded fan-out, multi-step workflows. Read when designing a job, writing `job.yaml`, triggering a job from the app or an agent, or escalating to K8s Jobs via `runner: k8s_job`. Pairs with the `/deploy_job` command.                                                   |
...
| [deployment.md](deployment.md)                     | App, agent, MCP server, and compute job deployment targets (Vercel, Vertex AI Agent Engine, Cloud Run, Cloud Run Jobs / per-tenant GKE). Read when pushing to main, running `/deploy_agent` / `/deploy_mcp` / `/deploy_job` / `/deploy_workflow`, or explaining how code reaches production.                                         |
